...
class CV2RTSPReader(threading.Thread):
    ...
    def _try_open_cap(self) -> cv2.VideoCapture | None:
        """
        VideoCapture ni thread ichida ochadi.
        RailSafeGUI yondashuvi: thread + join(timeout) — bloklanmaydi.
        """
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = _RTSP_OPTIONS
        ...
        def _worker():
            try:
                ...
                # Birinchi valid frameni topish (RailSafeGUI: 10 urinish)
        ...
        t = threading.Thread(target=_worker, daemon=True, name="RTSPOpen")
        t.start()
    ...
    def _open(self) -> cv2.VideoCapture | None:
        """
        Uch usul bilan urinib ko'rish:
          1. TCP + stimeout (asosiy)
          2. UDP fallback
          3. CAP_ANY (oxirgi chora)
        """
        # 1-urinish: TCP (asosiy, yuqorida)
